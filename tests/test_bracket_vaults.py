...
def test_water_barrier_on_row3(seed):
    ...
    for c in range(1, _BRACKET_VAULTS_COLS - 1):
        if c == _BRACKET_VAULTS_BRACKET_OPEN or c == _BRACKET_VAULTS_BRACKET_CLOSE:
...
def test_par_is_correct():
    """Sanity check: par=_BRACKET_VAULTS_PAR=8 and answer matches the constant for all seeds.
