...
def _add_char_run(room, row, col, length, kind='ancient'):
    ru = CharRun(row=row, col=col, symbols=tuple('∘' * length), kind=kind)
...
def test_w_from_start_of_rune_goes_to_start_of_next_rune():
    room = _make_room()
    _add_char_run(room, row=2, col=5, length=3)    # occupies 5-7
    _add_char_run(room, row=2, col=12, length=2)   # occupies 12-13
...
def test_w_from_middle_of_rune_goes_to_start_of_next_rune():
    room = _make_room()
    _add_char_run(room, row=2, col=5, length=3)
    _add_char_run(room, row=2, col=12, length=2)
...
def test_w_skips_void_rune():
    ...
    _add_char_run(room, row=2, col=5, length=3, kind='ancient')
    _add_char_run(room, row=2, col=10, length=2, kind='void')
    _add_char_run(room, row=2, col=18, length=3, kind='ancient')
    player = Player(row=2, col=5)
...
def test_count_2_w_jumps_two_runes():
    ...
    room = _make_room(cols=40)
    _add_char_run(room, row=2, col=5, length=3)
    _add_char_run(room, row=2, col=12, length=3)
    _add_char_run(room, row=2, col=20, length=3)
...
def test_w_blocked_by_wall_between_runes():
    ...
    _add_char_run(room, row=2, col=18, length=3)
    player = Player(row=2, col=5)
...
def test_e_from_start_of_rune_goes_to_end_of_that_rune():
    room = _make_room()
    _add_char_run(room, row=2, col=5, length=4)   # occupies 5-8
    player = Player(row=2, col=5)
...
def test_e_from_end_of_rune_skips_to_end_of_next_rune():
    ...
    room = _make_room(cols=40)
    _add_char_run(room, row=2, col=5, length=3)    # end at col 7
    _add_char_run(room, row=2, col=12, length=4)   # end at col 15
...
def test_e_skips_void_rune_to_next_regular_rune_end():
    ...
    room = _make_room(cols=40)
    _add_char_run(room, row=2, col=5, length=3, kind='ancient')   # end at 7
    _add_char_run(room, row=2, col=12, length=2, kind='void')
    _add_char_run(room, row=2, col=18, length=4, kind='ancient')  # end at 21
    player = Player(row=2, col=7)   # at end of first rune
