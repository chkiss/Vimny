...
class TestComputeTextObject:
    def test_dw_exclusive_span(self):
        ...
        room.add_rune(RuneCluster(3, 6, ('b',), 'ancient'))
        p = _player(3, 2)
...
class TestDelete:
    def test_dw_removes_word_and_trailing_gap(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('a',), 'ancient'))   # word A
        room.add_rune(RuneCluster(3, 6, ('b',), 'ancient'))   # word B
        p = _player(3, 2)
        t = compute_text_object(p, {'op': 'd', 'motion': 'w', 'count': 1, 'motion_count': 1}, room)
        op_delete(room, p, t)
        assert room.rune_at(3, 2) is None        # A gone
        assert room.rune_at(3, 6) is not None     # B preserved
        assert p.col == 2                         # cursor at start of deletion

    def test_de_removes_word_only(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('a', 'b', 'c'), 'ancient'))
        room.add_rune(RuneCluster(3, 8, ('d',), 'ancient'))
        p = _player(3, 2)
        t = compute_text_object(p, {'op': 'd', 'motion': 'e', 'count': 1, 'motion_count': 1}, room)
        op_delete(room, p, t)
        assert all(room.rune_at(3, c) is None for c in (2, 3, 4))
        assert room.rune_at(3, 8) is not None     # next word untouched
    ...
    def test_dd_clears_row(self):
        ...
        t = compute_text_object(p, {'op': 'd', 'motion': 'line', 'count': 1}, room)
        op_delete(room, p, t)
        assert room.rune_at(3, 2) is None and room.rune_at(3, 6) is None
        assert p.row == 3
    ...
    def test_delete_splits_partial_cluster(self):
        ...
        op_delete(room, p, t)
        assert room.rune_at(3, 2) is not None     # 'a' remnant
        assert room.rune_at(3, 3) is None         # 'b' deleted
...
class TestYankSpacing:
    ...
    def test_linewise_paste_preserves_spacing(self):
        ...
        assert room.rune_at(4, 2) is not None        # A back at col 2
        assert room.rune_at(4, 6) is not None        # B back at col 6
        assert all(room.rune_at(4, c) is None for c in (3, 4, 5))   # gap survived
    ...
    def test_charwise_paste_preserves_relative_gap(self):
        ...
        assert room.rune_at(3, 11) is not None
        assert room.rune_at(3, 15) is not None
        assert all(room.rune_at(3, c) is None for c in (12, 13, 14))
    ...
    def test_paste_stops_at_wall(self):
        ...
        assert room.rune_at(3, 22) is not None
        assert room.rune_at(3, 23) is None            # wall, not written
...
class TestComputeTextObjectMore:
    ...
    def test_operator_count_multiplies_motion(self):
        room = _room()
        for c in (2, 6, 10, 14):
            room.add_rune(RuneCluster(3, c, ('x',), 'ancient'))
        p = _player(3, 2)
    ...
    def test_df_inclusive_with_target(self):
        room = _room()
        room.add_rune(RuneCluster(3, 8, (';',), 'ancient'))
        p = _player(3, 2)
        t = compute_text_object(p, _op('d', 'f', target=';'), room)
        assert t.type is TextObjectType.INCLUSIVE
...
class TestYankNonMutation:
    def test_yank_does_not_change_room_or_cursor(self):
        ...
        clip = op_yank(room, p, t)
        assert room.rune_at(3, 2) is not None and room.rune_at(3, 4) is not None
...
class TestDeleteMore:
    def test_db_cursor_to_start(self):
        ...
        assert room.rune_at(3, 2) is None and room.rune_at(3, 6) is not None
        assert p.col == 2

    def test_dj_clears_both_rows_and_repositions(self):
        room = _room()
        room.add_rune(RuneCluster(2, 3, ('a',), 'ancient'))
        room.add_rune(RuneCluster(3, 5, ('b',), 'ancient'))
        p = _player(2, 5)
...
class TestPasteMore:
    def test_P_before_places_at_cursor(self):
        ...
        assert room.rune_at(3, 10) is not None
    ...
    def test_linewise_paste_below_with_p(self):
        ...
        assert room.rune_at(4, 2) is not None and room.rune_at(4, 6) is not None
        assert p.row == 4
...
class TestRoundTrip:
    def test_yank_then_paste_reproduces_layout(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('x',), 'ancient'))
        room.add_rune(RuneCluster(3, 5, ('y', 'z'), 'verdant'))   # gap at 3,4
        p = _player(3, 1)
        clip = op_yank(room, p, compute_text_object(p, _op('y', 'line'), room))
        p.row, p.col = 5, 1
        op_paste(room, p, clip, before=True)        # overlay onto blank row 5
        assert room.rune_at(5, 2) is not None        # x at same offset
        assert room.rune_at(5, 5) is not None        # yz at same offset
        assert all(room.rune_at(5, c) is None for c in (3, 4))   # gap preserved
...
class TestCaseOperators:
    ...
    def test_gUw_via_compute(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('a', 'b', 'c'), 'ancient'))
        room.add_rune(RuneCluster(3, 7, ('x',), 'ancient'))
        p = _player(3, 2)
    ...
    def test_gU_linewise(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('a',), 'ancient'))
        room.add_rune(RuneCluster(3, 8, ('b', 'c'), 'ancient'))
        p = _player(3, 8)
...
class TestIndent:
    ...
    def test_indent_moves_all_clusters_equally(self):
        ...
        room.add_rune(RuneCluster(3, 6, ('b',), 'ancient'))
        apply_indent(room, 3, INDENT_WIDTH)
        assert room.rune_at(3, 4) is not None and room.rune_at(3, 8) is not None
        assert room.rune_at(3, 2) is None and room.rune_at(3, 6) is None
...
class TestChangeWordSpecialCase:
    ...
    def test_cW_coalesces_to_WORD_end(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('a', 'b'), 'ancient'))   # adjacent → one WORD
        room.add_rune(RuneCluster(3, 4, ('c', 'd'), 'verdant'))
        room.add_rune(RuneCluster(3, 8, ('e',), 'ancient'))
        p = _player(3, 2)
        t = compute_text_object(p, _op('c', 'W'), room)
...
class TestFindOperators:
    ...
    def _cluster(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('a', 'b', ';', 'c', 'd'), 'ancient'))   # ; at col 4
        return room
    ...
    def test_dt_then_delete_removes_prefix(self):
        ...
        assert room.rune_at(3, 4) is not None             # ';' survives
...
class TestChangeComposition:
    ...
    def test_ce_deletes_word_and_lands_cursor_for_insert(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('o', 'l', 'd'), 'ancient'))
        p = _player(3, 2)
    ...
    def test_cc_clears_line_and_lands_at_first_passable(self):
        room = _room()
        room.add_rune(RuneCluster(3, 2, ('a',), 'ancient'))
        room.add_rune(RuneCluster(3, 8, ('b',), 'ancient'))
        p = _player(3, 8)
        t = compute_text_object(p, _op('c', 'line'), room)
        op_delete(room, p, t)
        assert room.rune_at(3, 2) is None and room.rune_at(3, 8) is None
