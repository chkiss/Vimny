...
def advance(src: pathlib.Path, chars: set) -> float:
    """The widest advance among `chars`, as a fraction of the em."""
    font = TTFont(src, fontNumber=0, lazy=True)
    cmap, hmtx, upm = font.getBestCmap(), font['hmtx'], font['head'].unitsPerEm
    return max((hmtx[cmap[c]][0] / upm for c in chars if c in cmap), default=0.0)
...
def main() -> int:
    ...
    css, remaining, cell = [], set(keep), 0.0
    for family, filename, is_mono in FACES:
        ...
        # The monospace face defines the cell. The fallbacks are proportional
        # fonts — Symbola draws a pentagram a full em wide, two thirds again
        # bigger than the cell — and a terminal emulator draws each glyph at
        # the cell origin regardless, so an oversized one paints over its
        # neighbour. Shrinking the whole face until its widest glyph fits is
        # the only lever there is: xterm.js positions by cell and ignores
        # advances entirely, so nothing else here would move the ink.
        widest = advance(src, got)
        if is_mono:
            cell = widest
            adjust = None
        else:
            adjust = round(cell / widest * 100) if widest > cell > 0 else None

        note = f'  scaled to {adjust}%' if adjust else ''
        print(f'   {family:12} {len(got):4} glyphs  {size / 1024:6.1f} KB{note}')
        css.append((family, woff2, adjust))
    ...
    body = '\n'.join(
        ...
        + f'}}\n'
        for family, fname, adjust in css)
