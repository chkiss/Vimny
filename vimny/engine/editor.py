...
from vimny.engine.world import (
...
#: remembered to extend it, and it had no way to SAY what it knew. Sunken water
...
def _ed_restore(room, player, snap: dict) -> None:
    ...
    # Sunken ground is two facts about a cell; an undo that restored only the
...
def _ed_paint(room, r, c, kind: str) -> bool:
    ...
    if sunken:
        ...
        room.underwater_cells.discard((r, c))
...
#: Underwater ground, in a shared level file. The haze is a property OF a cell
#: — it draws as one, it is painted as one — so it rides the grid rather than a
#: second list of coordinates that could disagree with it. Not a CellType, and
#: so not in `_CELL_CODE`: only `sharing.format`'s row codec knows it, and only
#: there does it split back into WATER + an underwater entry. The GLYPH is 'M'
#: for history (the layer was called "mist" until 2026-08-23); existing files
#: in the wild name it, and a file format never orphans its own.
_UNDERWATER_CODE = 'M'
assert _UNDERWATER_CODE not in _CODE_CELL
_MIST_CODE = _UNDERWATER_CODE          # historical alias
...
_ENTITY_FIELDS = ('kind', 'row', 'col', 'hp', 'max_hp', 'ai', 'ai_speed',
                  ...
                  'drops', 'group', 'opaque', 'lit', 'password')
...
def _deserialize_room(data: dict):
    ...
    room.underwater_cells  = {tuple(p) for p in data.get('underwater_cells', ())}
...
def _serialize_room(room) -> dict:
    ...
    return {
        ...
        'underwater_cells':   sorted(list(p) for p in room.underwater_cells),
