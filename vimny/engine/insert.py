...
from vimny.engine.world import CellType, CharRun, blocked_by_entity
from vimny.engine.motion import _first_non_blank_col, _leftmost_passable
from vimny.engine.editor import _merge_adjacent_char_runs, _split_run_at
from vimny.engine.reflow import is_ledge, open_gap, close_gap, extend_floor, _insert_blank_row
...
def _clear_row(room, row: int) -> None:
    ...
    from vimny.engine.operator import line_extent, _delete_cols
...
def insert_char(room, player, ch: str, kind: str = INSERT_KIND) -> bool:
    ...
    past the void brink falls in (see vimny/engine/reflow.py).
