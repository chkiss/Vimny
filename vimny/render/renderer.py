...
from vimny.engine.world import Dungeon, CellType, Room, CharRun, Entity, entity_letter
from vimny.engine.player import Player
from vimny.engine.modes import Mode, MODE_LABELS
from vimny.engine.visual import in_selection as _in_visual_sel
from vimny.engine.search import match_cells as _match_cells, find_next as _find_next
        ...
        if (room_r, room_c) in room.fog_cells:
            if (room.cells[room_r][room_c] == CellType.WATER
                    ...
                    and (_discovered(room, room_r, room_c)
                         or room._char_runs_by_row.get(room_r))):
                # UNDERWATER water reads as hazy water, not stone — the channel
                # it draws when discovered, ALWAYS when its row carries ink:
                # a decorative pool inside a written sheet is part of the
                # writing's scenery, not a secret of the bank.
