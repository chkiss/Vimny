...
Extend the famous-text program (`docs/blueprints/sense_not_decree.md`, pool
`vimny/content/proverbs.py`) into this wing: wherever a level would otherwise carve a
...
| III  | `"-`         | A single wrong word in a MISQUOTE (`vimny/content/proverbs.py`): the small charwise cut that fixes it survives in `"-` past an unrelated linewise `dd`. The famous line is the cue. |
...
+ `guard_message`; add `vimny/render/vim_commands.md` rows (hint tiers auto-derive).
...
- Add the nine + boss `LEVELS` entries as a separate bonus section (not on the main
  chain); regen the curriculum table (`vimny/content/_gen_curriculum_table.py`).
...
- Wizard-wisdom POEM per level (`vimny/art/_gen_wizard_wisdom.py`, `introduces_slug`).
...
3. **Saddlebag persistence key** — store the `"*` clip in `progress['saddlebag']`;
   confirm it should also survive vimny/save/quit (proposed: yes — it's the horse's).
4. **Text picks** — choose the specific proverbs/verse per level from the
   `vimny/content/proverbs.py` pool (extend it as needed): the II stanza (line order must
