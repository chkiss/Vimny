...
The shipped curriculum lives in `vimny/content/levels.py` (canonical) and is listed in
...
builder in `vimny/generation/dungeon_gen.py`, next to the code they explain.
...
Blueprint: `docs/blueprints/registry_wing.md`. Two of its levels have shipped (The
...
### Folds
Blueprint: `docs/blueprints/bonus_wing.md` — The Cartographer's Table, `zf`.
...
Blueprint: `docs/blueprints/level_sharing.md`; author guide: `docs/AUTHORING.md`;
code: the `vimny/sharing/` package. The load-bearing decisions held: a level is
...
The same replayer audits the shipped curriculum (`python3 -m vimny.sharing audit`) and
...
`vimny/content/levels.py` — never touch a slug — then rerun
`python3 vimny/content/_gen_curriculum_table.py` and the test suite. See
